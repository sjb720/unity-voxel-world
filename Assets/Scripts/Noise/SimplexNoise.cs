...
namespace SimplexNoise
{
    ...
    public static class Noise
    {
        ...
        public static float[,] Calc2D(int x, int z, int width, int height, float scale)
        {
            var values = new float[width, height];
            for (var i = 0; i < width; i++)
                for (var j = 0; j < height; j++)
                    values[i, j] = Generate((i+x) * scale, (j+z) * scale) * 128 + 128;
            return values;
        }
        ...
        public static float[, ,] Calc3D(int x, int z, int width, int height, int length, float scale)
        {
            var values = new float[width, height, length];
            for (var i = 0; i < width; i++)
                for (var j = 0; j < height; j++)
                    for (var k = 0; k < length; k++)
                        values[i, j, k] = Generate((i+x) * scale, j * scale, (k+z) * scale) * 128 + 128;
            return values;
        }
